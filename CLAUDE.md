...
This project uses three skills that form a complete development workflow:

| Situation | Invoke Skill |
|-----------|--------------|
| User reports bug, error, or unexpected behavior | `debugging` |
| Writing or modifying code (features, fixes, refactors) | `development` |
| Task complete, ready to finish | `finishing-work` |

**The standard chain:** `debugging` → `development` → `finishing-work`

- **debugging**: Systematic investigation using logs, Docker, database, and browser automation. Identifies root cause but does NOT write fixes.
- **development**: TDD workflow (RED → GREEN → REFACTOR). Write a failing test first, then implement.
- **finishing-work**: Runs `mix precommit`, records changes to PENDING.md. Required before claiming any task is done.

## Mandatory Workflows

### Code Changes Require TDD

**Invoke the `development` skill before writing production code.** All behavioral changes follow test-first development—no exceptions.

### Task Completion Requires Verification

**Invoke the `finishing-work` skill before telling the user you're done.** This runs quality checks and records changes. Without it, commits will fail validation.
...
GenServers should be thin integration layers. Business logic lives in companion `Core` modules that are pure functions and easily testable. See `TESTING.md` and the `development` skill for details and examples.
...
Target **85%+ code coverage**. See `TESTING.md` for full details and the `development` skill for TDD workflow.

## Logging

See `LOGGING.md` for full details. Use `Msfailab.Trace` for request/response dumps in development.
