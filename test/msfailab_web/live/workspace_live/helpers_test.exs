...
defmodule MsfailabWeb.WorkspaceLive.HelpersTest do
  ...
  # ===========================================================================
  # Track List Update Tests
  # ===========================================================================

  describe "update_tracks_list/2" do
    test "removes track when event has archived_at" do
      tracks = [
        %{id: 1, name: "Track One", slug: "track-one"},
        %{id: 2, name: "Track Two", slug: "track-two"}
      ]

      event = %{track_id: 1, archived_at: ~U[2024-01-01 00:00:00Z]}

      result = Helpers.update_tracks_list(tracks, event)

      assert length(result) == 1
      assert Enum.at(result, 0).id == 2
    end

    test "updates track name and slug when not archived" do
      tracks = [
        %{id: 1, name: "Old Name", slug: "old-slug"},
        %{id: 2, name: "Other Track", slug: "other"}
      ]

      event = %{track_id: 1, name: "New Name", slug: "new-slug", archived_at: nil}

      result = Helpers.update_tracks_list(tracks, event)

      assert length(result) == 2
      updated = Enum.find(result, &(&1.id == 1))
      assert updated.name == "New Name"
      assert updated.slug == "new-slug"
    end

    test "leaves other tracks unchanged" do
      tracks = [
        %{id: 1, name: "Track One", slug: "one"},
        %{id: 2, name: "Track Two", slug: "two"}
      ]

      event = %{track_id: 1, name: "Updated", slug: "updated", archived_at: nil}

      result = Helpers.update_tracks_list(tracks, event)

      other = Enum.find(result, &(&1.id == 2))
      assert other.name == "Track Two"
      assert other.slug == "two"
    end

    test "handles empty tracks list" do
      result =
        Helpers.update_tracks_list([], %{track_id: 1, archived_at: nil, name: "X", slug: "x"})

      assert result == []
    end
  end

  describe "maybe_update_track/2" do
    test "updates track when id matches" do
      track = %{id: 1, name: "Old", slug: "old"}
      event = %{track_id: 1, name: "New", slug: "new"}

      result = Helpers.maybe_update_track(track, event)
    ...
    test "returns unchanged track when id does not match" do
      track = %{id: 2, name: "Original", slug: "original"}
      event = %{track_id: 1, name: "New", slug: "new"}
